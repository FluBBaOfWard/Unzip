...
static int loadAndDecompressZip(void *dest, const CentralFileHdr *cfHdr, FILE *zipFile, const int maxSize) {
	...
	if ( fillLocalZipHeader(&zipHead, zipFile) == 0 ) {
		...
		if ( ucSize > maxSize ) {
			strlcpy(zipError, "File to large!", sizeof(zipError));
			err = 1;
			fclose(zipFile);
			return err;
		}
		if ( zipHead.compressionMethod == 8 ) {		// 8 == DEFLATE compression.
			if ( (src = malloc(cSize)) ) {
		...
		} else if ( zipHead.compressionMethod == 0 ) {		// 0 == STORE, no compression.
			fseek(zipFile, zipHead.extraFieldLength + zipHead.fileNameLength, SEEK_CUR);
